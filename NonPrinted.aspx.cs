...
public partial class NonPrinted : System.Web.UI.Page
{
    ...
    private int? edit()
    {
        try
        {
            ...
            con1.Open();
            OdbcCommand command = new OdbcCommand(@"update dbo.NonPrintedRequest set
                                                                    deptName=?,
                                                                    deptNID=?,
                                                                    title=?, 
                                                                    email=?, 
            ...
            command.Parameters.Add(new OdbcParameter("@authors", txtAuthors.Value.Trim()));
            command.Parameters.Add(new OdbcParameter("@productName", txtproductName.Value.Trim()));
            ...
            command.Parameters.Add(new OdbcParameter("@city", pubCity.Items[pubCity.SelectedIndex].Value));
            command.Parameters.Add(new OdbcParameter("@PostBox", txtDepPost.Value.Trim()));
            command.Parameters.Add(new OdbcParameter("@subjectID", int.Parse(subjectID.Items[subjectID.SelectedIndex].Value)));
            ...
            command.Parameters.Add(new OdbcParameter("@totalNoEpisode", txttotalNoEpisode.Value.Trim()));
            ...
            foreach (string image in images)
            {
                ...
                command2.Parameters.Add(new OdbcParameter("@TYPE", 3));// for nonPrinted Type
            ...
            command3.Parameters.Add(new OdbcParameter("@TYPE", 31));// for Moic image in nonprinted Type
    ...
    public int insert()
    {
        try
        {
            ...
            OdbcConnection con1 = new OdbcConnection(ConfigurationManager.AppSettings["connectionStringReal"]);
            con1.Open();
            OdbcCommand command = new OdbcCommand(@"insert into dbo.NonPrintedRequest ( 
                                                                    ID, 
                                                                    deptName,
                                                                    deptNID,
                                                                    title, 
    ...
    private void getdata()
    {
        ...
        foreach (Tuple<string, string> element in getSubjects())
        {
            subjectID.Items.Add(new ListItem(element.Item2, element.Item1));
        }
    ...
    public static List<Tuple<string, string>> getTypes()
    {
        OdbcConnection con1 = new OdbcConnection(ConfigurationManager.AppSettings["connectionStringReal"]);
        con1.Open();
        OdbcCommand command = new OdbcCommand("SELECT TypeID,TypeName FROM NonPrintedPhysicalType order by TypeName ", con1);
    ...
    public static List<Tuple<string, string>> getSubjects()
    {
        OdbcConnection con1 = new OdbcConnection(ConfigurationManager.AppSettings["connectionStringReal"]);
        con1.Open();
        OdbcCommand command = new OdbcCommand("SELECT SubjID,Subj FROM NonPrintedSubject order by subj", con1);
        DataSet results = new DataSet();
        OdbcDataAdapter usersAdapter = new OdbcDataAdapter(command);
        usersAdapter.Fill(results);
        DataTable dt = results.Tables[0];
        ...
        foreach (DataRow row in dt.Rows)
        {
            list.Add(Tuple.Create<string, string>(row["SubjID"].ToString(), row["Subj"].ToString()));
        }
        return list;
    ...
    public void loadPrevData()
    {
        OdbcConnection con1 = new OdbcConnection(ConfigurationManager.AppSettings["connectionStringReal"]);
        con1.Open();
        OdbcCommand command = new OdbcCommand("SELECT * FROM dbo.NonPrintedRequest where id = ? ", con1);
        ...
        foreach (DataRow row in dt.Rows)
        {
            ...
            txtproductName.Value = row["producerName"].ToString();
            ListItem li = hijry.Items.FindByText(row["prodHDate"].ToString());
            li.Selected = true;
            li = gerg.Items.FindByText(row["prodADate"].ToString());
            li.Selected = true;
            li = pubCity.Items.FindByValue(row["city"].ToString());
            li.Selected = true;
            li = typeID.Items.FindByValue(row["typeID"].ToString());
            li.Selected = true;
            li = subjectID.Items.FindByValue(row["subjectID"].ToString());
            li.Selected = true;
            txttotalNoEpisode.Value = row["totalNoEpisode"].ToString();
            txtepisodeNo.Value = row["episodeNo"].ToString();
            txtAuthors.Value = row["authors"].ToString();
    ...
    private bool ValidCheack()
    {
        OdbcConnection con1 = new OdbcConnection(ConfigurationManager.AppSettings["connectionStringReal"]);
        con1.Open();
        OdbcCommand command = new OdbcCommand("SELECT * FROM dbo.NonPrintedRequest where id = ? and (note ='' or note is null) ", con1);
        command.Parameters.Add(new OdbcParameter("@id", ViewState["id"].ToString()));
        DataSet results = new DataSet();
        OdbcDataAdapter usersAdapter = new OdbcDataAdapter(command);
        usersAdapter.Fill(results);
